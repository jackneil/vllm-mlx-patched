---
description: "Parallel UX checks — standalone (generated 2026-04-25; upgrade jacked + re-run /jacked-setup to update)"
---
...
- **Stack**: Python-only backend ML inference engine — **no frontend, no UI, no browser-renderable surface**
- **Browser Tool**: none detected
- **Dev Server Port**: n/a
- **Component Paths**: none

## Credential Hints
none found — repo has no `.env*` files

## UX Focus Areas

This repo has no UI — `/ux` is rarely applicable here. If invoked anyway:
- Confirm with the user whether they meant the **arena admin UI** (`~/Github/hank-llm-arena/`) or the **secure-llm Tauri app** (`~/Github/hank-secure-llm/`) — both are separate repos with actual frontends.
- If they meant to validate this repo's HTTP behavior across "many models", redirect them to the private benchmark suite proposed in `~/Github/hank-llm-lab/docs/ideas/005-private-benchmarking-suite.md` instead.
- Do NOT spawn parallel browser agents for vllm-mlx-patched — there is nothing visual to test.
